...
extern UniValue DoZulgSpend(const CAmount nAmount, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str);
...
extern UniValue setzulgseed(const JSONRPCRequest& request);
extern UniValue getzulgseed(const JSONRPCRequest& request);
extern UniValue generatemintlist(const JSONRPCRequest& request);
extern UniValue searchdzulg(const JSONRPCRequest& request);
extern UniValue dzulgstate(const JSONRPCRequest& request);
