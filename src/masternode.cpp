...
{
    if (nHeight > 200 && nHeight <= 140000) return 50000 * COIN;
    if (nHeight > 140000 && nHeight <= 210000) return 100000 * COIN;
    if (nHeight > 210000) return 200000 * COIN;
...
CAmount CMasternode::GetMasternodePayment(int nHeight)
{
    ...
    return GetBlockValue(nHeight) * 90 / 100;
}
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 ULG tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 Ultragate tx -> 1 confirmation
